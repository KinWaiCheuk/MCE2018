...
for i in range(61):    
    ...
    X_train, X_test = triplet_generation(trn_bl_ivector.reshape(classes,3,600),neg_class_num=50)
    Anchor = X_train[:,0,:]
    Positive = X_train[:,1,:]
    Negative = X_train[:,2,:]
    ...
    history = model.fit([Anchor,Positive,Negative],y=Y_dummy,validation_data=([Anchor_test,Positive_test,Negative_test],Y_dummy2), batch_size=512, epochs=5)
...
##############################1st Report#####################################
    ...
    # Top-1 detector EER
    dev_trials_confusion = get_trials_label_with_confusion(dev_identified_label, dev_trials_label, dev2train, dev_trials )
    dev_EER_confusion,confusion_err = calculate_EER_with_confusion(dev_scores,dev_trials_confusion)
    filename='triplet_Dense600_' + str(i) + 'nd_err' + str(confusion_err) +'_C10'
...
fig.savefig('Task2_training_loss')