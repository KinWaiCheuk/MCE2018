...
def generate_triplet(x,y,testsize=0.3,ap_pairs=10,an_pairs=10):
    ...
    for data_class in sorted(set(data_xy[1])):
        ...
        A_P_pairs = random.sample(list(permutations(same_class_idx[same_class_sampleer_idx:same_class_sampleer_idx+15],2)),k=ap_pairs) #Generating Anchor-Positive pairs
...
for i in range(101):    
    ...
    X_train, X_test = generate_triplet(trn_ivector,trn_trials, ap_pairs=200, an_pairs=300,testsize=0.2)
    Anchor = X_train[:,0,:]
    Positive = X_train[:,1,:]
    Negative = X_train[:,2,:]
...
#################Report thrid score######################    
    ...
    if err*100 < 1.9:
        try:
            foldername = './task1/'
            filename='task1_triplet_Dense600_' + str(i) + 'nd_err' + str(err*100)
...
fig.savefig('Task_training_loss')
